      ...
      <div className="relative mx-auto flex min-h-[min(820px,88vh)] max-w-7xl flex-col justify-between px-4 pb-8 pt-28 sm:px-6 lg:px-8">
          ...
          <div className="max-w-3xl">
            ...
            <div className="min-h-[310px] sm:min-h-[330px]">
              ...
              <div className="mt-8 flex flex-wrap items-center gap-3">
                <Link href={slide.href} className="inline-flex h-12 items-center gap-2 rounded-xl bg-[#DB1F26] px-5 text-sm font-semibold text-white shadow-xl shadow-black/25 transition hover:-translate-y-0.5 hover:bg-[#DB1F26]">
          ...
          <aside className="hidden rounded-3xl border border-white/15 bg-black/25 p-3 shadow-2xl backdrop-blur-xl lg:block" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)}>
            ...
            <div className="space-y-1">
              {slides.map((item, index) => {
                ...
                return (
                  <button key={item.label} type="button" onClick={() => setActive(index)} className={`flex w-full items-center gap-3 rounded-2xl p-3 text-left transition ${selected ? "bg-[#DB1F26] text-white" : "text-white hover:bg-white/10 hover:text-white"}`}>
        ...
        <div className="flex items-center justify-between border-t border-white/15 pt-5">
          <div className="flex items-center gap-2">
            {slides.map((item, index) => <button key={item.label} type="button" aria-label={`Show ${item.label}`} onClick={() => setActive(index)} className={`h-1.5 rounded-full transition-all ${index === active ? "w-10 bg-[#DB1F26]" : "w-5 bg-white/30 hover:bg-white/60"}`} />)}
          </div>
          <div className="flex items-center gap-2">
