      ...
      <div className="relative mx-auto flex min-h-[min(820px,88vh)] max-w-7xl flex-col justify-between px-4 pb-8 pt-28 sm:px-6 lg:px-8">
        <div className="grid gap-12 lg:grid-cols-[minmax(0,1fr)_340px] lg:items-center">
          <div className="max-w-3xl">
            <div className="mb-7 flex flex-wrap items-center gap-3 text-[10px] font-semibold uppercase tracking-[0.2em] text-white">
              <span className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3.5 py-2 backdrop-blur-md"><ShieldCheck className="size-3.5 text-[#6A0B14]" /> Amata real estate agency</span>
              <span className="inline-flex items-center gap-1.5"><MapPin className="size-3.5 text-[#6A0B14]" /> Zimbabwe</span>
            </div>
            <div className="min-h-[310px] sm:min-h-[330px]">
              <p className="text-sm font-semibold uppercase tracking-[0.18em] text-[#6A0B14]">{slide.label}</p>
              ...
              <p key={`${slide.title}-description`} className="mt-6 max-w-xl animate-fade-in text-base leading-7 text-white sm:text-lg">
                {slide.description}
              </p>
              <div className="mt-8 flex flex-wrap items-center gap-3">
                <Link href={slide.href} className="inline-flex h-12 items-center gap-2 rounded-xl bg-[#6A0B14] px-5 text-sm font-semibold text-white shadow-xl shadow-black/25 transition hover:-translate-y-0.5 hover:bg-[#6A0B14]">
          ...
          <aside className="hidden rounded-3xl border border-white/15 bg-black/25 p-3 shadow-2xl backdrop-blur-xl lg:block" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)}>
            <div className="flex items-center justify-between px-3 pb-3 pt-2">
              <span className="text-[10px] font-semibold uppercase tracking-[0.18em] text-white">Our services</span>
              <span className="text-xs text-white">0{active + 1} / 0{slides.length}</span>
            </div>
            <div className="space-y-1">
              {slides.map((item, index) => {
                ...
                return (
                  <button key={item.label} type="button" onClick={() => setActive(index)} className={`flex w-full items-center gap-3 rounded-2xl p-3 text-left transition ${selected ? "bg-[#6A0B14] text-white" : "text-white hover:bg-white/10 hover:text-white"}`}>
                    ...
                    <span className="min-w-0"><span className="block text-sm font-semibold">{item.label}</span><span className={`mt-0.5 block truncate text-xs ${selected ? "text-white" : "text-white"}`}>{item.action}</span></span>
                  </button>
                );
              })}
            </div>
        ...
        <div className="flex items-center justify-between border-t border-white/15 pt-5">
          <div className="flex items-center gap-2">
            {slides.map((item, index) => <button key={item.label} type="button" aria-label={`Show ${item.label}`} onClick={() => setActive(index)} className={`h-1.5 rounded-full transition-all ${index === active ? "w-10 bg-[#6A0B14]" : "w-5 bg-white/30 hover:bg-white/60"}`} />)}
          </div>
          <div className="flex items-center gap-2">
